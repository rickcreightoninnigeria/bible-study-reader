...
// back to 🌐. Returns badge HTML instead of the raw flag when the entry
// carries a badge property.
function currentLangFlag() {
  const lang  = window.appLocale || resolveLanguage();
  const entry = LANGUAGE_MAP[lang];
  if (!entry) return '🌐';
  return renderLangBadge(entry);
}
...
// Any future same-flag language just needs a badge: { letter, bg } entry —
  ...
  function tabLanguage() {
    ...
    const groupsHtml = groups.map(group => `
      <div class="settings-block" style="margin-bottom:10px;">
        ...
        >
          <span class="settings-lang-flag">${renderLangBadge(lang)}</span>
