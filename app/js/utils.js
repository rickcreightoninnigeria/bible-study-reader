...
function saveLastPosition() {
  ...
  safeSetItem(_lastPositionKey(), JSON.stringify(pos));
}
...
// All localStorage writes in the app go through safeSetItem rather than calling
// localStorage.setItem directly. This guards against QuotaExceededError (storage
// full) and SecurityError (private-browsing / WebView restrictions), both of
// which would otherwise surface as unhandled exceptions in the UI.
//
// On failure the write is silently dropped and a warning is logged. Answer data
// is the highest-value payload; if storage is full the user should clear old
// studies rather than lose the current write silently — but a console warning is
// preferable to a crash that leaves the whole onchange handler broken.
//
